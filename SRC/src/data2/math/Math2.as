package data2.math
{
	...
	public class Math2 {
		...
		static public function getProgressionValue(_input:Number, _minsrc:Number, _maxsrc:Number, _mindst:Number, _maxdst:Number):Number
		{
			var _percentinput:Number = (_input - _minsrc) / (_maxsrc - _minsrc);
			if (_percentinput < 0) _percentinput = 0;
			if (_percentinput > 1) _percentinput = 1;
			var _output:Number = _mindst + (_maxdst - _mindst) * _percentinput;
			return _output;
		}
		
		
		static public function getRandProbability(_frequency:Number):Boolean
		{
			if (_frequency <= 0) return false;
			if (_frequency >= 1) return true;
			
			var _denom:Number = Math2.round((1 / _frequency), 0.1);
			var _rand:int = Math2.random(0, _denom, 0.1);
			return (_rand < 1);
		}

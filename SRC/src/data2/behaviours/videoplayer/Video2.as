...
videoPlayer.addEventListener(Video2Event.META_DATA, onVideoMeta);
videoPlayer.addEventListener(Video2Event.FINISHED, onVideoFinished);
...
package data2.behaviours.videoplayer {
	...
	public class Video2 extends MovieClip{
		...
		public function Video2(_autoRewind:Boolean=true) 
		{ 
		...
		{
			trace("Video2.play " + currentTime + ", " + totalTime);
			...
			*/
			...
			this.dispatchEvent(new Video2Event(Video2Event.PLAY));
		}
		...
		public override function stop():void
		{
			...
			this.dispatchEvent(new Video2Event(Video2Event.STOP));
		}
		...
		private function openStream()
		{
			...
			client.onMetaData = function(obj:Object){
				...
				firstMetaData = false;
				var e:Video2Event = new Video2Event(Video2Event.META_DATA);
				e.obj = obj;
		...
		private function netStatusHandler(event:NetStatusEvent):void {
			...
			if (!_bloaded && this.download >= 99) {
				_bloaded = true;
				dispatchEvent(new Video2Event(Video2Event.LOADED));
			}
			...
			if(code=="NetConnection.Connect.Success"){
				...
				//vidéo play
				dispatchEvent(new Video2Event(Video2Event.BUFFER_STOP));
				...
				dispatchEvent(new Video2Event(Video2Event.FINISHED));
				
			}else{
				//buffer pas entièrement chargé
				dispatchEvent(new Video2Event(Video2Event.BUFFER_START));
			}

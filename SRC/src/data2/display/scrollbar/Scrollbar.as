...
package data2.display.scrollbar {
	...
	public dynamic class Scrollbar extends LayoutSprite {
		...
		//static private const THROW_FACTOR_RALENT:Number = 0.86;
		static private const THROW_FACTOR_RALENT:Number = 0.94;
		//static private const THROW_FACTOR:Number = 0.4;
		static private const THROW_FACTOR:Number = 1.0;
		static private const THROW_DELTA_CEIL:Number = 50;
		...
		private var _ptfoo:Point;
		private var _optimizeFunction:Function;
		...
		public function set optimizeFunction(value:Function):void { _optimizeFunction = value; }
		...
		{
			trace("Scrollbar.onresize");
			trace("needed : " + (_contentheight > _height + 0.1));
		...
		private function stopScroll(e:MouseEvent):void
		{
			...
			if (_touchMode) {
				...
				this.removeEventListener(Event.ENTER_FRAME, onEnterframeCaptureMove2);
				removeMouseListener(_stage, MouseEvent.MOUSE_MOVE, onEnterframeCaptureMove);
			}
		}
		...
		private function updateMC(e:MouseEvent):void
		{
			...
			if (_optimizeFunction != null) _optimizeFunction();
		}
		...
		private var _ptscrollmove:Point;
		
		private function onMouseDownTarget(e:Event):void 
		{
			
			if (_lock || !_scrollneeded) return;
			...
			_ptscrollmove = getPositionFromMouseEvent(e);
			_listScrollMove.push(_ptscrollmove.y);
			this.addEventListener(Event.ENTER_FRAME, onEnterframeCaptureMove2);
			addMouseListener(_stage, MouseEvent.MOUSE_MOVE, onEnterframeCaptureMove);
			
			trace("_touchPositionInit : " + _touchPositionInit);
		...
		private function onMouseUpTarget(e:Event):void 
		{
			...
			//trace("(scrollbar) _delta : " + _delta);
			
			if (_delta > THROW_DELTA_CEIL) _delta = THROW_DELTA_CEIL;
			else if (_delta < -THROW_DELTA_CEIL) _delta = -THROW_DELTA_CEIL;
			
			_delta *= THROW_FACTOR;
			_delta /= _contentheight;
			_delta *= 500;
		...
		private function onThrowEnterframe(e:Event):void 
		{
			...
			_touchThrowDelta *= THROW_FACTOR_RALENT;
			//trace("_touchThrowDelta : " + _touchThrowDelta);
			if (Math.abs(_touchThrowDelta) < 0.1) this.removeEventListener(Event.ENTER_FRAME, onThrowEnterframe);
		}
		...
		private function getDeltaTouchDrop():Number 
		{
			...
			//trace("_lsm : " + _listScrollMove + ", _lsm[_len - 1] : " + _listScrollMove[_len - 1] + ", _lsm[_firstindex] : " + _listScrollMove[_firstindex]);
		...
		private function onEnterframeCaptureMove2(e:Event):void 
		{
			_listScrollMove.push(_ptscrollmove.y);
		}
		
		private function getPositionFromMouseEvent(e:Event):Point 
		{
			if (_ptfoo == null) _ptfoo = new Point();
			if (e is TouchEvent) {
				var _te:TouchEvent = TouchEvent(e);
				_ptfoo.x = _te.stageX;
				_ptfoo.y = _te.stageY;
			...
			else throw new Error("e is not of goo type : " + e);
			
			var _position:Point = this.globalToLocal(_ptfoo);
			return _position;
		}

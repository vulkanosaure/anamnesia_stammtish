package data.form 
{
	import data.form.Checkbox;
	import data.form.CheckboxEvent;
	...
	public class RadioGroup extends EventDispatcher
	{
		...
		{
			if (!_item is Checkbox && !_item is RadioGroup) throw new Error("arg must be of type Checkbox or RadioGroup");
			if (_item is Checkbox) {
				Checkbox(_item).addEventListener(CheckboxEvent.CHECK, onCheckCB);
				Checkbox(_item).addEventListener(CheckboxEvent.UNCHECK, onUncheckCB);
			}
		...
		private function setSelection(_value:Boolean, _except:*=null):void
		{
			...
			for (var i:int = 0; i < _len; i++) {
				if (_items[i] != _except) {
					if (_items[i] is Checkbox) Checkbox(_items[i]).checked = _value;
		...
		private function onUncheckCB(e:CheckboxEvent):void 
		{
			...
			for (var i:int = 0; i < _len; i++) 
			{
				if (_items[i] is Checkbox && Checkbox(_items[i]).checked) return;
